<template>
  <div ref="demoBlock" :class="['demo-block', blockClass, customClass ? customClass : '', { hover }]"
    ...
    <div class="source" :style="{ background: colorList[activeColor] }">
      <div class="toggle-box" :style="{ color: activeColor === 'Light' ? '#212121' : '#e8e8e8' }">
        <div class="toggle-box__inner" @click="clickToggleColor">
          <div>Theme: </div>
          <label class="toggle-box__switch">
            <!-- <input class="toggle-box__input" type="checkbox" checked> -->
            <div class="toggle-box__div" :style="{ transform: activeColor === 'Light' ? 'translate3d(100%,0,0)' : '' }">
            </div>
          </label>
          <div style="margin-left: 10px;"> {{ activeColor }}</div>
          <div style="margin-left: 10px;"> {{ colorList[activeColor] }}</div>
        </div>
      </div>
...
export default {
  ...
  setup (props) {
    ...
    const clickToggleColor = (event) => {
      if (event) {
        event.stopPropagation();
      }
      if (activeColor.value === 'Dark') {
        activeColor.value = 'Light';
      } else {
        activeColor.value = 'Dark';
      }
      console.log('切换', activeColor.value);
    };
...
.toggle-box__div {
  ...
  border-radius: 16px;
  background: #0080ff;
  box-shadow: 0 1.6px 4.8px #0000004d;
  transition: all .3s;
}
