...
import { ref, reactive, computed, watch, onBeforeMount, onMounted, onUpdated, onBeforeUnmount, nextTick, } from 'vue';
...
const getBelowCount = computed((): number => {
  return Math.min(props.data.length - state.end, props.bufferScale * getVisibleCount.value);
});
...
const setStartOffset = () => {
  let startOffset = 0;
  if (state.start >= 1) {
    let size = state.positions[state.start].top - (
...
const updateItemsSize = () => {
  if (virtualList__item.value) {
    for (let i = 0; i < virtualList__item.value.length; i++) {
      ...
      // 存在差值
      if (diffHeight) {
        state.positions[index].bottom = state.positions[index].bottom - diffHeight;
  ...
  if (state.intervalID === null && props.autoScroll && !state.isMouseHover) {
    ...
    state.intervalID = setInterval(() => {
      if (virtualList.value) {
        let realScrollDistance = props.scrollDistance || state.positions[state.start].height;
        ...
        virtualList.value.scrollTo({
          top: virtualList.value.scrollTop + realScrollDistance,
          behavior: 'smooth'
        });
      }
